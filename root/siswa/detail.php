...
$query_DetailRS2 = sprintf("SELECT * FROM siswa WHERE id_siswa = %s", GetSQLValueString($colname_DetailRS2, "int"));
$query_limit_DetailRS2 = sprintf("%s LIMIT %d, %d", $query_DetailRS2, $startRow_DetailRS2, $maxRows_DetailRS2);
$DetailRS2 = mysqli_query($alijtihad_db, $query_limit_DetailRS2);
$row_DetailRS2 = mysqli_fetch_assoc($DetailRS2);

if (isset($_GET['totalRows_DetailRS2'])) {
  $totalRows_DetailRS2 = $_GET['totalRows_DetailRS2'];
} else {
  $all_DetailRS2 = mysqli_query($alijtihad_db,$query_DetailRS2);
  $totalRows_DetailRS2 = mysqli_num_rows($alijtihad_db,$all_DetailRS2);
}
